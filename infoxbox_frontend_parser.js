...
function getData() {
	...
	var values     = $('.kno-fv').map(function()  { return $(this).text(); }).get();
	var properties = $('._om').map(function() { return $(this).text(); }).get();
	...
	if ($('.kno-sh').first().size())
		{
			...
			specialPropertyValues = $('._Zh').first().find('a._dt').map(function(){ return $(this).text(); }).get();
